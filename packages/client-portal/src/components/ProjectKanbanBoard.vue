  ...
  <div class="kanban-container">
    ...
    <div class="kanban-header">
      <div class="header-left">
        <div>
          <h2 class="text-2xl font-bold text-gray-900">{{ projectName }} - Kanban Board</h2>
          <p class="text-gray-600 mt-1">Track progress and manage tasks for this project</p>
        </div>
    ...
    <div v-else-if="error" class="error-state">
      <h3 class="text-sm font-medium text-red-800">Error loading project tasks</h3>
      <p class="text-sm text-red-600">{{ error }}</p>
      <div class="mt-4 space-y-2">
        <button @click="loadProjectTasks" class="text-red-400 hover:text-red-600">
          Try again
        </button>
    ...
    <div v-else class="kanban-board">
      ...
      <div v-if="tasks.length === 0" class="no-data-state">
        ...
        <h3 class="no-data-title">No tasks found for this project</h3>
        <p class="no-data-description">
          {{ error ? 'There was an error loading tasks. Please try again.' : 'No tasks have been created for this project yet.' }}
        </p>
        <button v-if="error" @click="loadProjectTasks" class="retry-button">
          Try Again
        </button>
      </div>
...
const loadProjectTasks = async () => {
  ...
  console.log('🔍 Loading tasks for project ID:', props.projectId)
  
  // For now, use sample data based on project ID
  // This will work with the sample projects (id: 1, id: 2)
  const sampleTasks = {
    1: [
      {
        _id: 'task1',
    ...
    2: [
      {
        ...
        status: 'todo',
        priority: 'high',
        assignedTo: { firstName: 'Alex', lastName: 'Brown' },
        sprint: { name: 'Sprint 1' }
      },
      {
        _id: 'task6',
        ...
        status: 'in-progress',
        priority: 'high',
        assignedTo: { firstName: 'Chris', lastName: 'Davis' },
        sprint: { name: 'Sprint 1' }
      },
      {
        _id: 'task7',
  ...
  // Set project name based on ID
  const projectNames = {
    1: 'Website Redesign',
    2: 'Mobile App Development'
  }
  
  project.value = { name: projectNames[props.projectId] || 'Project' }
  tasks.value = sampleTasks[props.projectId] || []
  sprints.value = [
    { _id: 'sprint1', name: 'Sprint 1' },
    { _id: 'sprint2', name: 'Sprint 2' }
  ]
  
  console.log('🔍 Loaded sample tasks for project:', tasks.value.length)
  loading.value = false
}
...
const filterTasksBySprint = () => {
  ...
  if (!selectedSprint.value) {
    ...
    console.log('Filtered tasks by sprint:', filteredTasks.length)
  }
}
